...
class MessageAdapter(val context: Context, private val list: ArrayList<Messages>):RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        holder.itemView.setOnLongClickListener(object :View.OnLongClickListener{
            override fun onLongClick(p0: View?):Boolean {
                AlertDialog.Builder(context).setTitle("Delete message from me ?").setPositiveButton("Yes",object :DialogInterface.OnClickListener{
                    override fun onClick(p0: DialogInterface?, p1: Int) {
                        val ref=Firebase.database.reference.child("chats").child(msg.uId.toString()+msg.receiverId)
